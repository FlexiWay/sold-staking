...
const bigIntToFloat = (bigIntValue: bigint | number, decimals: number): number => {
...
export const useSold = () => {
    ...
    const getUserBalances = async () => {
        ...
        const userBase = findAssociatedTokenPda(umi, { owner: umi.identity.publicKey, mint: poolManager.baseMint });
        const userXMint = findAssociatedTokenPda(umi, { owner: umi.identity.publicKey, mint: poolManager.xMint });
        ...
        console.log("Fetched Balances:", userBaseAtaAcc?.amount, userXmintAtaAcc?.amount, userXmintAtaAcc?.amount); // Debug fetched balances including xMint

        setUserBalancePUSD(userBaseAtaAcc ? bigIntToFloat(userBaseAtaAcc.amount, tokenManager.mintDecimals) : 0);
        setUserBalanceSPUSD(userXmintAtaAcc ? bigIntToFloat(userXmintAtaAcc.amount, tokenManager.quoteMintDecimals) : 0);
    }
    useEffect(() => {
        getUserBalances();
    ...
    const annualYieldRate = poolManager ? (Number(poolManager.annualYieldRate) / 100).toString() : '0';
    ...
    return { tokenManager, poolManager, refetch, loading, statCardData, userBalancePUSD, userBalanceSPUSD, amount, setAmount, exchangedAmount, calculateExchangedAmount, exchangeRate, handleStake, handleUnstake, annualYieldRate, stakingTab, setStakingTab };
};